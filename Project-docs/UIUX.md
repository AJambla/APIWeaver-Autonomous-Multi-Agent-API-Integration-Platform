...
## 3. Vesper.ai React+Vite Frontend (Single-Page Application)

**Goal:** Replace the existing Next.js frontend with a React+Vite single-page app delivering the same functionality with a pure black aesthetic and no external dependencies beyond Vite, React, and TypeScript.

### 3.1 Architecture
- **Framework:** Vite + React 18 + TypeScript (no Next.js routing, App Router replaced by client-side routing via `react-router-dom` hash-based navigation for simplicity).
- **Styling:** Inline CSS with CSS Modules for scoped styles; no external CSS framework.
- **API Layer:** Shared TypeScript `api.ts` client wrapper (same as before) using native `fetch` with interceptors for auth token refresh.
- **State Management:** React Context for auth, global notifications, and UI state; no Redux.
- **Routing:** Hash-based (`#dashboard`, `#projects/123`) to avoid server-side routing complexity.
- **Build:** `npm run build` produces a static `dist` folder served by `vite preview` or Nginx.
...
│   │   ├── Button.tsx            # Liquid-metal pill button
│   │   ├── Card.tsx              # Card container
│   │   ├── Toast.tsx             # Notification system
│   │   └── ...
│   ├── lib/
│   │   ├── auth-context.tsx     # Auth provider (same logic as before)
...
│   │   ├── projects/
│   │   │   ├── [id]/page.tsx     # Project detail hub
│   │   │   ├── [id]/build/page.tsx
│   │   │   ├── [id]/test/page.tsx
│   │   │   └── ...
│   │   └── auth/
...
│   └── favicon.svg                # Provided data URI
├── index.html
├── vite.config.ts
└── package.json
```

### 3.3 Core Visual Requirements
- **Background:** `html, body { background: #000000 !important; color: #ffffff; }` + `style="background:#000;color:#fff"` on `<body>`.
- **Fonts:** Self-hosted WOFF2 Inter (100-900) and Instrument Serif Italic placed in `public/` folder.
- **Logo:** Same SVG mark (white) as provided data URI.
- **No extra images** — only the hero video MP4 and the favicon data URI.

### 3.4 Hero Section (Single Viewport)
- Full-viewport black background with 100% opacity video playing muted loop.
- Centered headline: "Train AI agents on your workflows in minutes." with `em` for "AI agents" in Instrument Serif.
- Subtext: "Deploy adaptive AI agents that learn, execute, and scale operational tasks."
- Two primary buttons: Solid "Start for Free" and Ghost "See it in action".
- No pricing tables, no extra sections, no footer beyond three stats.

### 3.5 Stats Footer
Three equal-width stats with icons (workflow count, reduction metric, teams count) matching the design tokens.

### 3.6 Navigation
- Top bar with logo left, three nav links (Benefits, How It Works, FAQs) center, CTA right.
- Mobile burger menu for <901px.
- All links hash-based (`#benefits`, `#how-it-works`, etc.).

### 3.7 Implementation Notes
- **CSS-in-JS:** No external CSS-in-JS libraries; use inline `<style>` blocks in components.
- **Performance:** Lazy load route components; code-split via Vite.
- **Accessibility:** WCAG 2.1 AA, keyboard navigation, focus rings, reduced motion support.
- **No service worker** — the 404 request is harmless and not part of the spec.

### 3.8 Build & Serve
- `npm run build` → `dist` folder.
- Serve with `npx serve dist` or Nginx.
- Environment variables via `.env` file (same as backend).

---

*Document generated by Kilo on 2026-09-23. Follow the exact specifications above for implementation.*
